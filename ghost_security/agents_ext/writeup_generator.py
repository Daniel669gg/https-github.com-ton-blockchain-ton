"""
Ghost Security — TON Bug Bounty Public Writeup Generator
...
class WriteupGenerator:
    ...
    def generate(self, finding: Dict,
        ...
        lines += [
            ...
            f"",
            f"- {ts}: Vulnerability discovered via Ghost Security Platform",
            f"- {ts}: Self-check validation completed (score: {finding.get('_check_score','N/A')}/100)",
            f"- TBD: Reported to TON Security Team",
            f"- TBD: Fix deployed",
            f"",
            f"---",
            f"*This report was generated with [Ghost Security Platform](https://ghost-security.io) "

"""
Ghost Security Platform — GitHub App Integration
...
class GitHubClient:
    ...
    def create_check(self, owner: str, repo: str, sha: str, name: str = "Ghost Security") -> str:
...
def findings_to_annotations(findings: List[dict]) -> List[dict]:
    ...
    for f in findings:
        ...
        ann  = {
            ...
            "title":            f.get("rule_id") or f.get("type") or "Ghost Security",
...
def pr_summary_comment(result: PRScanResult) -> str:
    ...
    lines = [
        f"## {icon} Ghost Security — PR #{result.pr_number} Scan Results",
        "",
    ...
    lines += [
        "",
        "_[Ghost Security Platform](https://github.com/ghost-security/platform) — defensive AppSec scanning_",
    ]
    return "\n".join(lines)

...
# ---------------------------------------------------------------------------
# Additional inference cache tests
# ---------------------------------------------------------------------------

class TestAdditionalCacheBehavior:
    def test_stats_has_size_bytes(self, cache):
        stats = cache.stats()
        assert "size_bytes" in stats

    def test_stats_has_total_gets_and_hits(self, cache):
        stats = cache.stats()
        assert "total_gets" in stats
        assert "total_hits" in stats

    def test_total_gets_increments_on_miss(self, cache):
        h = InferenceCache.hash_prompt("model", "not_there")
        cache.get(h)
        stats = cache.stats()
        assert stats["total_gets"] >= 1

    def test_total_hits_increments_on_hit(self, cache):
        h = InferenceCache.hash_prompt("model", "hit_me")
        cache.set(h, "response")
        cache.get(h)
        stats = cache.stats()
        assert stats["total_hits"] >= 1

    def test_total_entries_zero_initially(self, cache):
        stats = cache.stats()
        assert stats["total_entries"] == 0

    def test_set_multiple_entries_counted(self, cache):
        for i in range(5):
            h = InferenceCache.hash_prompt("model", f"prompt_{i}")
            cache.set(h, f"response_{i}")
        stats = cache.stats()
    ...
    def test_hash_prompt_is_static_method(self):
        # Can be called without an instance
        h = InferenceCache.hash_prompt("m", "p")
        assert len(h) == 64

    def test_unicode_prompt_hashed(self, cache):
        h = InferenceCache.hash_prompt("model", "こんにちは世界")
        cache.set(h, "Japanese response")
        result = cache.get(h)
        assert result == "Japanese response"

    def test_unicode_response_stored(self, cache):
        h = InferenceCache.hash_prompt("model", "hello")
        response = "Привет мир 🌍"
        cache.set(h, response)
        assert cache.get(h) == response

    def test_size_bytes_positive_after_insert(self, cache):
        h = InferenceCache.hash_prompt("model", "x")
        cache.set(h, "response")
        stats = cache.stats()
        assert stats["size_bytes"] > 0

    def test_get_returns_none_for_completely_different_hash(self, cache):
        result = cache.get("a" * 64)
        assert result is None

    def test_set_then_evict_then_miss(self, short_ttl_cache):
        h = InferenceCache.hash_prompt("model", "expiry_test")
        short_ttl_cache.set(h, "response")
        time.sleep(0.01)
        short_ttl_cache.evict_expired()
        result = short_ttl_cache.get(h)
        assert result is None

    def test_multiple_misses_total_gets_tracks(self, cache):
        for i in range(5):
            h = InferenceCache.hash_prompt("model", f"miss_{i}")
            cache.get(h)
        stats = cache.stats()
        assert stats["total_gets"] >= 5

    def test_hit_rate_approaches_1_on_all_hits(self, cache):
        h = InferenceCache.hash_prompt("model", "repeated")
        cache.set(h, "resp")
        for _ in range(10):
            cache.get(h)
        stats = cache.stats()

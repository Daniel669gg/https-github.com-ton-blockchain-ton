...
# ---------------------------------------------------------------------------
# Additional APIKeyManager tests
# ---------------------------------------------------------------------------

class TestAdditionalAPIKeyManager:
    def test_create_key_multiple_orgs(self, key_manager):
        r1 = key_manager.create_key("org1", "K1", ["read"])
        r2 = key_manager.create_key("org2", "K2", ["write"])
        assert r1["key"] != r2["key"]

    def test_validate_key_returns_key_id(self, key_manager):
        result = key_manager.create_key("org1", "K1", ["read"])
        meta = key_manager.validate_key(result["key"])
        assert meta["key_id"] == result["key_id"]

    def test_validate_key_returns_name(self, key_manager):
        result = key_manager.create_key("org1", "My Key", ["read"])
        meta = key_manager.validate_key(result["key"])
        assert meta["name"] == "My Key"

    def test_list_keys_contains_key_id(self, key_manager):
        result = key_manager.create_key("org1", "K1", ["read"])
        keys = key_manager.list_keys("org1")
        key_ids = [k["key_id"] for k in keys]
        assert result["key_id"] in key_ids

    def test_list_keys_contains_name(self, key_manager):
        key_manager.create_key("org1", "SpecialName", ["read"])
        keys = key_manager.list_keys("org1")
        names = [k["name"] for k in keys]
        assert "SpecialName" in names

    def test_list_keys_multiple(self, key_manager):
        key_manager.create_key("org1", "K1", ["read"])
        key_manager.create_key("org1", "K2", ["write"])
        key_manager.create_key("org1", "K3", ["admin"])
        keys = key_manager.list_keys("org1")
        assert len(keys) >= 3

    def test_revoked_key_not_in_validate(self, key_manager):
        result = key_manager.create_key("org1", "K1", ["read"])
        key_manager.revoke_key(result["key_id"], "org1")
        assert key_manager.validate_key(result["key"]) is None

    def test_rotate_preserves_scopes(self, key_manager):
        old = key_manager.create_key("org1", "K1", ["read", "write"])
        new = key_manager.rotate_key(old["key_id"], "org1")
        meta = key_manager.validate_key(new["key"])
        assert set(meta["scopes"]) == {"read", "write"}

    def test_rotate_preserves_name(self, key_manager):
        old = key_manager.create_key("org1", "MyKey", ["read"])
        new = key_manager.rotate_key(old["key_id"], "org1")
        assert new["name"] == "MyKey"

    def test_create_key_with_empty_scopes(self, key_manager):
        result = key_manager.create_key("org1", "K1", [])
        assert result["scopes"] == []

    def test_create_key_with_multiple_scopes(self, key_manager):
        scopes = ["read", "write", "admin", "delete"]
        result = key_manager.create_key("org1", "K1", scopes)
...
class TestAdditionalSessionManager:
    def test_create_session_with_metadata(self, session_manager):
        token = session_manager.create_session("user1", "org1", {"role": "admin"})
        result = session_manager.validate_session(token)
        assert result is not None
        assert result["metadata"].get("role") == "admin"

    def test_session_has_expires_at(self, session_manager):
        token = session_manager.create_session("user1", "org1")
        result = session_manager.validate_session(token)
        assert "expires_at" in result

    def test_session_has_session_id(self, session_manager):
        token = session_manager.create_session("user1", "org1")
        result = session_manager.validate_session(token)
        assert "session_id" in result
        assert result["session_id"]

    def test_multiple_sessions_same_user(self, session_manager):
        t1 = session_manager.create_session("user1", "org1")
        t2 = session_manager.create_session("user1", "org1")
        r1 = session_manager.validate_session(t1)
        r2 = session_manager.validate_session(t2)
        assert r1 is not None
        assert r2 is not None
        assert r1["session_id"] != r2["session_id"]

    def test_revoke_one_does_not_affect_other(self, session_manager):
        t1 = session_manager.create_session("user1", "org1")
        t2 = session_manager.create_session("user1", "org1")
        session_manager.revoke_session(t1)
        assert session_manager.validate_session(t1) is None
        assert session_manager.validate_session(t2) is not None

    def test_refresh_preserves_user_id(self, session_manager):
        old_token = session_manager.create_session("user1", "org1")
        new_token = session_manager.refresh_session(old_token)
        result = session_manager.validate_session(new_token)
        assert result["user_id"] == "user1"

    def test_refresh_preserves_org_id(self, session_manager):
        old_token = session_manager.create_session("user1", "org1")
        new_token = session_manager.refresh_session(old_token)
        result = session_manager.validate_session(new_token)
        assert result["org_id"] == "org1"

    def test_token_format_is_base64_dot_separated(self, session_manager):
        token = session_manager.create_session("user1", "org1")
        parts = token.split(".")
        assert len(parts) == 3
    ...
    def test_validate_revoked_session_returns_none(self, session_manager):
        token = session_manager.create_session("user1", "org1")
        session_manager.revoke_session(token)
        assert session_manager.validate_session(token) is None

    def test_refresh_revoked_session_returns_none(self, session_manager):
        token = session_manager.create_session("user1", "org1")
        session_manager.revoke_session(token)

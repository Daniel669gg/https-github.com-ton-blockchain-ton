"""
Ghost Security Platform — Autonomous Remediation Engine
...
class PRCreator:
    ...
    def create_fix_pr(
        ...
        push = gh("PUT", f"/repos/{owner}/{repo}/contents/{patch.file}", {
            "message": f"fix: {patch.finding_type} security vulnerability\n\n"
                       f"Automated fix by Ghost Security Platform.\n"
        ...
        pr = gh("POST", f"/repos/{owner}/{repo}/pulls", {
            "title":  f"[Ghost Security] Fix {patch.finding_type} in {Path(patch.file).name}",
    ...
    def _pr_body(patch: RemediationPatch) -> str:
        return f"""## 🔒 Ghost Security — Automated Fix
...
*Generated by [Ghost Security Platform](https://github.com/ghost-security/platform)*
"""

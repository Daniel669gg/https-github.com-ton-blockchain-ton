...
def _validate_yaml_structure(rule_path: str) -> bool:
    """Fast in-process YAML structure check — no subprocess overhead."""
    try:
...
def _validate_with_semgrep(rule_path: str) -> bool:
    """Validate rule: fast YAML structure check first, then semgrep if needed."""
    # Fast in-process check first (no subprocess startup cost)
    if not _validate_yaml_structure(rule_path):
        logger.warning("semgrep --validate FAILED for %s:\n%s", rule_path,
                       "Invalid YAML structure (missing 'rules' or 'id')")
        return False
...
class RuleGenerator:
    ...
    def _build_rule(
        ...
        # Fast YAML structure validation only — avoids subprocess overhead per rule
        _validate_yaml_structure(str(rule_file))

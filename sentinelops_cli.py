...
""",
    ...
    # ── Phase 19: Evolution to Semgrep/Snyk/CodeQL level ──────────────────────
    p_copilot = sub.add_parser("copilot", help="Autonomous Security Copilot — Scan→Analyze→Patch cycle")
    p_copilot.add_argument("target", help="File or directory to analyze")
    p_copilot.add_argument("--threshold", type=float, default=0.5, metavar="CONFIDENCE",
                           help="Confidence threshold (default: 0.5)")

    p_gap = sub.add_parser("gap-analysis", help="GAP analysis vs Semgrep, Snyk, CodeQL, Wiz")

    p_js = sub.add_parser("js-scan", help="JavaScript/TypeScript taint analysis")
    p_js.add_argument("target", help="JS/TS file or directory to scan")

    p_kg = sub.add_parser("kg-build", help="Build Security Knowledge Graph from scan findings")
    p_kg.add_argument("target", help="Findings JSON file to build graph from")
    ...
    # ── Phase 19: Autonomous Copilot & GAP Analysis ──────────────────────────

    def cmd_copilot(args) -> int:
        """Run the Autonomous Security Copilot full cycle on a file/directory."""
        from backend.agents.security_copilot import SecurityCopilot
        target = args.target
        print(BLUE(f"Security Copilot — full cycle on {target}"))
        try:
            copilot = SecurityCopilot()
            report = copilot.full_cycle(target)
        except FileNotFoundError as exc:
            print(RED(f"Error: {exc}"))
            return 1
        except Exception as exc:
            print(RED(f"Copilot error: {exc}"))
            return 1

        findings_count = report.get("findings_count", 0)
        risk = report.get("overall_risk", "UNKNOWN")
        print(f"  Findings: {findings_count}")
        print(f"  Overall risk: {_SEV_COLOR.get(risk, lambda t: t)(risk)}")
        ...
        if sev:
            ...
            for level in ("CRITICAL", "HIGH", "MEDIUM", "LOW"):
                ...
                if count:
                    print(f"    {_SEV_COLOR.get(level, lambda t: t)(level)}: {count}")

        patches = report.get("patches", [])
        if patches:
            print(f"  Patches generated: {len(patches)}")
            pct = report.get("patch_coverage_pct", 0.0)
            print(f"  Patch coverage: {pct:.0f}%")
    ...
    def cmd_gap_analysis(args) -> int:
        ...
        print(BLUE("TythanAI GAP Analysis vs Competitors"))
    ...
    def cmd_js_scan(args) -> int:
        """Scan JavaScript/TypeScript files for taint vulnerabilities."""
        from backend.scanners.js_taint_analyzer import JSTaintAnalyzer
        target = args.target
        print(BLUE(f"JS/TS Taint Scan: {target}"))
        ...
        print(RED(f"Found {len(findings)} issue(s):"))
        for f in findings:
            sev_fn = _SEV_COLOR.get(f.severity, lambda t: t)
            print(f"  {sev_fn(f.severity)} {f.rule_id} at {f.file}:{f.line} — {f.description[:80]}")
        return 1

    def cmd_kg_build(args) -> int:
        """Build a Security Knowledge Graph from a scan result JSON file."""
        import json as _json
        from backend.analysis.knowledge_graph import KnowledgeGraphBuilder
        from backend.core.confidence import findings_from_dicts
        target = args.target
        print(BLUE(f"Building Security Knowledge Graph from {target}"))
        try:
            raw = _json.loads(Path(target).read_text())
            findings = findings_from_dicts(raw if isinstance(raw, list) else raw.get("findings", []))
        except Exception as exc:
            print(RED(f"Error reading findings: {exc}"))
            return 1

        builder = KnowledgeGraphBuilder()
        graph = builder.build(project_root=".", findings=findings)
        node_count = len(graph.nodes)
        edge_count = len(graph.edges)
        print(GREEN(f"✓ Graph built: {node_count} nodes, {edge_count} edges"))
    ...
    dispatch = {
        ...
        # TythanAI Phase 19 — Evolution to Semgrep/Snyk/CodeQL level
        "copilot":      cmd_copilot,
        "gap-analysis": cmd_gap_analysis,
        "js-scan":      cmd_js_scan,
        "kg-build":     cmd_kg_build,
    }

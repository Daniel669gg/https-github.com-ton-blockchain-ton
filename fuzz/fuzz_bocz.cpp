// Standalone fuzzer for TON compressed-BoC decompression.
// Targets boc_decompress / boc_decompress_baseline_lz4 / _improved_structure_lz4:
// attacker-controlled compressed bytes -> size header parse -> LZ4 decompress ->
// custom structural cell reconstruction -> BoC deserialize. Reachable from
// compressed broadcasts / block transfer. The "improved structure" codec rebuilds
// the cell DAG from a compact bit format by hand - a prime spot for memory bugs.
...
constexpr unsigned kMax = 1u << 21;  // 2 MiB decompressed cap
...
void run_one(const unsigned char* data, size_t size) {
  ...
  { auto r = vm::boc_get_algorithm_name(s); (void)r; }
  { auto r = vm::boc_need_state_for_decompression(s); (void)r; }
  { auto r = vm::boc_decompress_baseline_lz4(s, kMax); (void)r; }
  { auto r = vm::boc_decompress_improved_structure_lz4(s, kMax); (void)r; }
  { auto r = vm::boc_decompress(s, (int)kMax); (void)r; }
...
void add_compressed(const std::vector<td::Ref<vm::Cell>>& roots) {
  auto push = [&](td::Result<td::BufferSlice> r) {
    if (r.is_ok()) { auto b = r.move_as_ok(); g_seeds.emplace_back(b.as_slice().data(), b.size()); }
  };
  push(vm::boc_compress_baseline_lz4(roots));
  push(vm::boc_compress_improved_structure_lz4(roots));
  push(vm::boc_compress(roots));
}
void build_seeds() {
  add_compressed({vm::CellBuilder().finalize()});
  add_compressed({mk("10101010")});
  add_compressed({mk(std::string(1023, '1'))});
  add_compressed({mk("1010", {mk("11110000"), mk(std::string(300, '0'))})});
  add_compressed({mk("1", {mk("0"), mk("1"), mk("00"), mk("11")})});
  {
    td::Ref<vm::Cell> c = mk("1");
    for (int i = 0; i < 30; i++) c = mk("01", {c});
    add_compressed({c});
  }
  {
    auto shared = mk("101010101010");
    add_compressed({mk("0", {shared, mk("1", {shared})})});
  }
  // multi-root
  add_compressed({mk("111"), mk("000"), mk("1", {mk("0")})});
...
void mutate(std::string& s) {
  ...
  for (int i = 0; i < rounds; i++) {
    ...
    switch (rnd(9)) {
      ...
      case 6: if (s.size()>=4){ size_t p=rnd(s.size()-3); uint32_t v=(uint32_t)xrand(); memcpy(&s[p],&v,4);} break;   // size header / counts

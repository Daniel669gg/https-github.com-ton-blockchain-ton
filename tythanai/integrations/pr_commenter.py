"""
TythanAI Platform — PR Inline Comment Integration
...
class PRCommenter:
    """
    Posts TythanAI findings as inline PR / MR review comments.
    ...
    def _format_single_finding(self, finding: dict) -> str:
        ...
        lines += [
            "",
            "<sub>Posted by [TythanAI](https://github.com/ghost-security/platform) "
            "— Security Gate</sub>",
        ]
        return "\n".join(lines)
    ...
    def _format_comment_group(self, findings: List[dict]) -> str:
        ...
        parts: List[str] = [
            f"**TythanAI: {len(findings)} findings on this line**",
            "",
        ]
    ...
    def _build_summary_body(
        ...
        header_lines = [
            f"## {gate_icon} TythanAI Security Scan — PR #{pr_number}",
            "",
        ...
        if total == 0:
            ...
            header_lines += [
                "",
                "<sub>_[TythanAI](https://github.com/ghost-security/platform) "

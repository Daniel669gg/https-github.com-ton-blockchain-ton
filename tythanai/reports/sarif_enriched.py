"""
TythanAI Platform — SARIF 2.1.0 Enriched Exporter
...
class SARIFExporter:
    """
    Convert TythanAI findings to SARIF 2.1.0.
...
def generate_github_actions_workflow(scan_type: str = "all") -> str:
    """Return a GitHub Actions YAML that runs TythanAI on push/PR."""
    return f"""name: TythanAI Scan
...
jobs:
  ghost-security:
    ...
    steps:
      ...
      - name: Install TythanAI Platform
        run: |
          pip install -r requirements.txt
          pip install semgrep bandit

      - name: Run TythanAI scan
        id: ghost_scan
        env:
      ...
      - name: Post PR comment
        ...
        with:
          script: |
            const fs = require('fs');
            let summary = 'TythanAI scan complete.';
            try {{
